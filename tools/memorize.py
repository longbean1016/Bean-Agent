...
class MemorizeTool(Tool):
    ...
    async def execute(
        ...
        **_runtime_context: Any,
        ...
        # 额外参数属于工具运行上下文，不能作为记忆字段保存；扩展字段须显式放入 metadata。
        if metadata is not None and not isinstance(metadata, dict):
            return "错误：记忆元数据必须是 JSON 对象，未写入记忆。"
        payload = dict(metadata) if metadata is not None else {}
        if tool_requirement is not None:
            payload["tool_requirement"] = tool_requirement
        if steps is not None:
            payload["steps"] = steps
        try:
            serialize_memory_metadata(payload)
        except ValueError as error:
            return f"错误：{error}，未写入记忆。"
        ...
        result = await self._memory.mutate(MemoryMutation(
            kind="remember",
            summary=summary,
