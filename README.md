...
[🎯 项目介绍](#-项目介绍) · [⚡ 并发与会话](#-多会话并发与独立页面) · [🔔 主动能力](#-主动提醒与主动聊天) · [🚀 快速开始](#-快速开始) · [📁 项目结构](#-项目结构)
...
- 🔗 **独立会话页面**：每个会话拥有独立 URL、文字草稿和附件草稿，支持直接刷新以及浏览器前进、后退
...
- `/`：新会话页面；发送第一条消息后自动进入对应的会话 URL
- `/chat/{id}`：已有会话页面；支持直接访问、刷新和浏览器前进、后退
- 每个页面分别保存文字与附件草稿，切换会话时不会相互覆盖
- 新会话首次发送后立即出现在会话列表中；默认标题取自用户问题，只有图片、文件或混合附件时分别使用 `分析图片内容`、`分析文件内容`、`分析附件内容`
...
- **主动聊天**：Agent 根据会话上下文和主动程度判断是否发起对话；“克制、均衡、积极”表示算法倾向，不保证固定次数
