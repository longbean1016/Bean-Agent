...
it("成功、失败、中断和未知状态分开统计", () => {
  ...
  expect(summary.displayLabel).toBe("1 项已中断 · 1 项状态未知 · 1 / 4 已完成");
});
it("并行调用仍在执行时保留失败提示，并支持动态增加工具", () => {
  ...
  expect(summary.displayLabel).toContain("1 项失败 · 1 / 3 已完成");
  expect(JSON.stringify(summary)).not.toContain("测试错误");
});
