...
@pytest.mark.asyncio
async def test_consolidation_uses_one_event_prompt_for_shared_source() -> None:
    provider = Provider()
    ...
    assert {name for name, _ in provider.calls} == {"submit_consolidation_events"}
...
@pytest.mark.asyncio
async def test_consolidation_retries_event_function_once() -> None:
    ...
    assert draft.history_entries


@pytest.mark.asyncio
async def test_consolidation_does_not_start_recent_context_extraction() -> None:
